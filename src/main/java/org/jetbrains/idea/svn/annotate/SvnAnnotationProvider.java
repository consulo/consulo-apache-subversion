...
import consulo.ide.impl.idea.openapi.vcs.annotate.VcsRareLineAnnotationData;
...
public class SvnAnnotationProvider implements AnnotationProvider, VcsCacheableAnnotationProvider {
    ...
    Runnable command = new Runnable() {
      public void run() {
        ...
        try {
          ...
          if (info == null) {
            exception[0] = new VcsException(new SVNException(SVNErrorMessage.create(SVNErrorCode.UNKNOWN,
                                                                                    "File ''{0}'' is not under version control",
                                                                                    ioFile)));
            return;
          }
  ...
  private SvnRemoteFileAnnotation annotateNonExisting(Pair<SvnChangeList, FilePath> pair,
                                                      ...
                                                      Info info,
                                                      Charset charset,
                                                      final VirtualFile current) throws VcsException, SVNException, IOException {
                     ...
                     new LogEntryConsumer() {
                       ...
                       public void consume(LogEntry logEntry) {
                         ...
                         myResult.setRevision(logEntry.getRevision(),
                                              new SvnFileRevision(myVcs, SVNRevision.UNDEFINED, logEntry, myUrl, ""));
                       }
                     });
    }
