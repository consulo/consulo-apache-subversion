...
public class SvnFileUrlMappingImpl implements SvnFileUrlMapping, PersistentStateComponent<SvnMappingSavedPart> {
  ...
  @Inject
  public SvnFileUrlMappingImpl(final Project project, final ProjectLevelVcsManager vcsManager) {
    myProject = project;
  ...
  private class NewRootsApplier {
    ...
    private void notifyRootsReloaded(boolean mappingsChanged) {
      ...
      if (mappingsChanged || !myInitedReloaded) {
        ...
        bus.syncPublisher(SvnVcs.ROOTS_RELOADED).rootsReloaded(true);
        bus.syncPublisher(ProjectLevelVcsManager.VCS_CONFIGURATION_CHANGED_IN_PLUGIN).directoryMappingChanged();
      }
      else {
        bus.syncPublisher(SvnVcs.ROOTS_RELOADED).rootsReloaded(false);
      }
    }
  }
  ...
  public String getRootForPath(final File currentPath) {
    ...
    convertedPath =
      (currentPath.isDirectory() && (!convertedPath.endsWith(File.separator))) ? convertedPath + File.separator : convertedPath;
    synchronized (myMonitor) {
  ...
  public void loadState(final SvnMappingSavedPart state) {
    ((ProjectLevelVcsManagerImpl)ProjectLevelVcsManager.getInstance(myProject)).addInitializationRequest(VcsInitObject.AFTER_COMMON,
                                                                                                         (DumbAwareRunnable)() -> ApplicationManager
                                                                                                           .getApplication
                                                                                                             ()
