...
public class SvnConfiguration implements PersistentStateComponent<SvnConfigurationState> {
  ...
  public void setIgnoreSpacesInAnnotate(final boolean value) {
    ...
    if (changed) {
      getProject().getMessageBus().syncPublisher(VcsAnnotationRefresher.class).configurationChanged(SvnVcs.getKey());
    }
  }
  ...
  private static Long fixSupportedVersion(final Long version) {
    return version == null || version.longValue() < CHANGELIST_SUPPORT ? UPGRADE_TO_15_VERSION_ASKED : version;
  }
  ...
  public void getServerFilesManagers(final Ref<SvnServerFileManager> systemManager,
                                     final Ref<SvnServerFileManager> userManager) {
    ...
    systemManager.set(new SvnServerFileManagerImpl(new IdeaSVNConfigFile(new File(SVNFileUtil.getSystemConfigurationDirectory(),
                                                                                  IdeaSVNConfigFile.SERVERS_FILE_NAME))));
